...
app.controller('preview', function($scope,$http,$location,$sce) {
	...
	$http.get("http://localhost/api/docs/"+$scope.id).
		then(function(response) {
			...
			if(["image/jpeg","image/png","image/tiff","image/gif"].indexOf($scope.content.type)!=-1){
				...
				$scope.content.filename = "https://docs.google.com/gview?embedded=true&url=http://localhost/content/"+response.data.content[0].local;
				$scope.download_url = "http://localhost/download/"+response.data.content[0].local;
				if($scope.goodBrowsers)
					$scope.content.filename = "http://localhost/content/"+response.data.content[0].local;
			}
			else{
			...
			if($scope.content.type=="images"){		
				...
				for(i=0; i<response.data.content.length; i++){
					if(i==0){ $scope.content.now = "http://localhost/content/"+response.data.content[0].local; }
					$scope.content.images.push({active: (i==0), src: "http://localhost/content/"+response.data.content[i].local});
				}
